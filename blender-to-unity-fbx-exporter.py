...
def export_unity_fbx(context, filepath, active_collection, selected_objects, deform_bones, leaf_bones, primary_bone_axis, secondary_bone_axis, tangent_space, triangulate_faces):
    ...
    # Store original location of the active object
	active_obj_name = bpy.context.view_layer.objects.active.name if bpy.context.view_layer.objects.active else None
	original_location = bpy.context.view_layer.objects.active.location.copy() if bpy.context.view_layer.objects.active else None
	
    # print the original location
	print("Original location:", original_location)

    # Move active object to the origin
	if active_obj_name:
		bpy.data.objects[active_obj_name].location = (0, 0, 0)
	...
	try:
		...
		if active_obj_name:
			...
			if active_obj:
				...
				print("Restored location for:", active_obj_name)
	...
	# Always ensure the active object is moved back to its original location before concluding the function
	# Ensure the final restoration and cleanup logic also re-fetches the active object
	if active_obj_name:
		active_obj = bpy.data.objects.get(active_obj_name)
		if active_obj:
			active_obj.location = original_location
			print("Finally restored location for:", active_obj_name)
